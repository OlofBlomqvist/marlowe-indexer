...
pub fn bound_values(filter:&Option<NumFilter>,contract:&Contract) -> bool {
    if let Some(bounds_filter) = filter {
        ...
        return if let Some(d) = datum_to_check {
            let numer_of_bounds = d.state.bound_values.len();
            num_check(bounds_filter, numer_of_bounds as f64)
        } else {
            false // no match
        }                
    }
...
pub fn locked_funds(filter:&LockedAmountFilter,contract:&Contract) -> bool {
    ...
    let current_transition = 
        contract.transitions.last().expect("marlowe-indexer bug: there must be at least one transition or there would be no contract");
    ...
    let mut transformed_accounts_in_state = vec![];
    let accs : AccMap<(marlowe_lang::types::marlowe::Party,marlowe_lang::types::marlowe::Token),u128> = current_datum.state.accounts.clone();
    for ((account_owner,token),amount) in accs.iter() {
        if *amount > 0 {
            match account_owner {
                marlowe_lang::types::marlowe::Party::Address(a) => {
                    transformed_accounts_in_state.push(
                        LockedAmountsResult {
                            account_owner_role: None,
                            account_owner_addr: Some(a.as_bech32().expect("marlowe-rs bug: failed to convert address to bech32")),
    ...
    for current_account in transformed_accounts_in_state  {

        // Filter by amounts
        let amounts_match = num_check_opt(&filter.amount, current_account.amount);
                
        // Filter by currency_symbol
        let symb_match = str_check_opt(&filter.currency_symbol, &current_account.currency_symbol);
        
        // Filter by token_name
        let tok_name_match = str_check_opt(&filter.token_name, &current_account.token_name);
            ...
            Some(af) => {
                match af {
                    AccountOwnerFilter::Role(role_filter) => {
                        if let Some(aor) = &current_account.account_owner_role {
                    ...
                    AccountOwnerFilter::Address(addr_filter) => {
                        if let Some(aoa) = &current_account.account_owner_addr {
