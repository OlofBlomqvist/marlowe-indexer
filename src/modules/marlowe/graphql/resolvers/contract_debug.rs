...
#[cfg(feature="debug")]
pub(crate) fn marlowe_rs_test(contract:&crate::modules::marlowe::graphql::resolvers::Contract,magic:u64) -> String {
    ...
    for window  in contract.transitions.windows(2) {
        ...
        if let [first, second] = window {
            ...
            let slot_time = slot_to_posix_time_with_specific_magic(second.abs_slot.u64(), magic);
            let time_interval_end = slot_to_posix_time_with_specific_magic(second.validity_range.1.expect("cant run contracts without ttl"),magic)  - 1;
   ...
   // println!("MARLOWE-RS TEST OF CONTRACT {} WAS SUCCESSFULL!",self.short_id);
    "OK".into()
}
...
#[cfg(feature="debug")]
pub(crate) fn describe_complete(contract:&crate::modules::marlowe::graphql::resolvers::Contract,magic:u64) -> String {
    use crate::core::lib::slot_to_posix_time_with_specific_magic;
    ...
    match &d.contract {
        marlowe_lang::types::marlowe::Contract::Close => {                
            ...
            return if locked_amounts.is_empty() {
                String::from("This contract is of type 'Close'. The only possible transaction remaining is one that completes the contract.")
            } else {
                format!("This contract is of type 'Close'. The only possible transaction remaining is one that completes the contract. The transaction that completes this contract must distribute the remaining assets : {locked_amounts:#?}")
            }
        },
        _ => {}
    };
    ...
    let time_of_previous_tx = slot_to_posix_time_with_specific_magic(x.abs_slot.u64(), magic);

    let mut contract_instance = marlowe_lang::semantics::ContractInstance::from_datum(d).with_current_time(time_of_previous_tx);
    ...
    match &b {
        marlowe_lang::semantics::MachineState::Closed => {
            let locked = d.state.locked_amounts();
                ...
                locked.iter().filter(|(_owner,_token,amount)| *amount > 0).collect();
            return if locked_amounts.is_empty() {
                String::from("Pending Close: The only possible transaction remaining is one that completes the contract. This state is expected to occur very rarely on-chain.")
            } else {
                format!("Pending Close: The only possible transaction remaining is one that completes the contract. The transaction that completes this contract must distribute the remaining assets : {locked_amounts:#?}")
            }
        }
