...
use crate::modules::marlowe::state::{Contract, MarloweTransition};


// TODO -> Add support for filtering by expected inputs
//         so that we can allow oracles to search for interesting contracts?
...
impl Contract {
    ...
    async fn describe_debug(&self) -> String {
        crate::modules::marlowe::graphql::resolvers::contract_debug::describe_complete(self,1)
    }
    ...
    async fn marlowe_rs_test(&self) -> String {
        ...
        super::contract_debug::marlowe_rs_test(self,1)
    }
    ...
    async fn describe(&self) -> Result<Option<String>,String> {
        ...
        if let Some(d) = &current_utxo.datum {
            ...
            //let slot = current_utxo.abs_slot as u64;
            let contract_instance = marlowe_lang::semantics::ContractInstance::from_datum(&d);
            // .with_current_time(
            //     slot_to_posix_time_with_specific_magic(slot, magic)
            // );
    ...
    async fn next_time_out(&self) -> Result<Option<f64>,String> {
       ...
       // let time_of_previous_tx = slot_to_posix_time_with_specific_magic(x.abs_slot as u64, magic);
    
        if let Some(ei) = &x.datum {
           let machine = marlowe_lang::semantics::ContractInstance::from_datum(ei);//.with_current_time(time_of_previous_tx);
    ...
    async fn transitions(&self,filter:Option<TransitionFilter>) -> Vec<&MarloweTransition> {
        let results = self.transitions.iter().filter(|x| {
            ...
            if let Some(f) = &filter {
                ...
                if let Some(slot_filter) = f.slot { 
                    if slot_filter as u64 != x.abs_slot { return false } 
                }

                if let Some(block_num_filter) = f.block_num { 
                    if block_num_filter as u64 != x.block_num { return false } 
                }
