...
use crate::state::{OrderedContracts, Contract};
...
async fn test_invalid_before_arg() {
    ...
    assert_eq!(result.err().unwrap(), crate::graphql::query::QueryError::BeforeNotFound);
}
...
async fn test_invalid_after_arg() {
    ...
    assert!(result.is_err());
    assert_eq!(result.err().unwrap(), crate::graphql::query::QueryError::AfterNotFound);
}
